...
function insertNews()
{
    ...
    $description = isset($_POST['description']) ? utf8_decode($_POST['description']) : "";
    ...
    if ($status['status'] == 'success')
    {
        ...
        $sql      = "INSERT INTO " . $g->news[$lan] . " (title, review_text, thumbnail_image, news_details_description, news_date, data) VALUES ('$title', '$review_text', '$fileName', '$description','$news_date', '$news_data')";
        $lan      = $lan == 0 ? 1 : 0;
        $sql1     = "INSERT INTO " . $g->news[$lan] . " (title, review_text, thumbnail_image, news_details_description, news_date, data) VALUES ('', '', '$fileName', '', '', '')";
...
function insertColumnistsArticles()
{
    ...
    $status = checkImageUploadError($_FILES['thumbnail'], 'columnists_articles_');
    $images = grabeImages($_FILES['app_image']);
    ...
    if ($status['status'] == 'success')
    {
        ...
        $g        = new General();
        $lan      = $_SESSION['language'];
        $sql      = "INSERT INTO " . $g->column_articles[$lan] . " (columnists_id, title, thumbnail_image, article_date, description, data) VALUES ('$columnist_id', '$title', '$fileName', '$article_date', '$description', '$data')";
        $lan      = $lan == 0 ? 1 : 0;
...
function insertRaceCourses()
{
    ...
    if ($status['status'] == 'success')
    {
        ...
        else
        {
            ...
            {
                $coverPhoto = $status['message'];

                if (!move_uploaded_file($_FILES['cover_photo']['tmp_name'], '../images/' . $coverPhoto))
                {
...
function insertRaceCourseRaces()
{
    ...
    $fileNames   = "";
     ...
     if (empty($image_error))
     {
        ...
        if ($pdfExt == "") 
        {
            $sql               = "INSERT INTO " . $g->race[$lan] . " (race_course_id, race_title, race_start_time, race_end_time, race_held_date, weather, track_condition, safety_limit, rail_position, description, date_record_entered)
                        VALUES ('$race_course_id', '$title', '$start_time', '$end_time', '$held_date', '$weather_condition', '$track_condition', '$safety_limit', '$rail_position', '$description', '$date_recorded')";
            $lan               = $lan == 0 ? 1 : 0;
            ...
            $sql               = "INSERT INTO " . $g->race[$lan] . " (race_course_id, race_title, race_start_time, race_end_time, race_held_date, weather, track_condition, safety_limit, rail_position, description, data, date_record_entered)
                        VALUES ('$race_course_id', '$title', '$start_time', '$end_time', '$held_date', '$weather_condition', '$track_condition', '$safety_limit', '$rail_position', '$description', '{$pdfFilePath}', '$date_recorded')";
            $lan               = $lan == 0 ? 1 : 0;
 ...
 ///   $imagesExt = pathinfo($_FILES["pdf_link1"]["name"], PATHINFO_EXTENSION);
    ...
    $fileNames   = "";
     ...
     if (empty($image_error))
     {
        ...
        if (!move_uploaded_file($_FILES["pdf_link1"]['tmp_name'], $saveToPath))
        {
            die("Problem in uploding PDF file...");
        }
        else {
            if ($cover['status'] == 'success')
            {
...
function updatePublications()
{
    ...
    $fileNames   = "";
     ...
     if (empty($image_error))
     {
            ...
            $pdfFilePath = substr($baseurl, 0,strlen($baseurl)-6) . 'images/pdf/publications/' . $fileNames . $imagesExt;
            $saveToPath = '../images/pdf/publications/' . $fileNames . $imagesExt;
             move_uploaded_file($_FILES["pdf_link1"]['tmp_name'], $saveToPath);
...
function updateGeneraldetails()
{
    ...
    $lan        = $_SESSION['language'];
    $post_id    = $_POST['post_id'];
    $about      = utf8_decode($_POST['about']);
    $web        = $_POST['website'];
    $credits    = $_POST['credits'];
    $copyrights = utf8_decode($_POST['copyrights']);
...
function insertRaceCourseTrack()
{
    ...
    $track_specification = isset($_POST['description']) ? utf8_decode($_POST['description']) : "";
    $data                = isset($_POST['data']) ? $_POST['data'] : "";
...
function updateThumbnail($table_e, $table_a, $label, $redirect)
{
    ...
    if ($status['status'] == 'success')
    {
        ...
        if ($result1 && $result2)
        {
            if ($table_e == "contact_us") {
                $baseurl = $base_url;
                echo "<script>window.location.replace('".$baseurl."contact.php?action_request=edit_records');</script>";
            }
            else if ($redirect == "race_courses") {
                $baseurl = $base_url;
                echo "<script>window.location.replace('".$baseurl."race_courses.php?action_request=edit_records&id=".$post_id."');</script>";
            }
            else if ($redirect == "race_course_track") {
                $baseurl = $base_url;
                echo "<script>window.location.replace('".$baseurl."race_course_track.php?action_request=edit_records&id=".$post_id."');</script>";
            }
            else if ($redirect == "general_news") {
                $baseurl = $base_url;
                echo "<script>window.location.replace('".$baseurl."news.php?action_request=edit_records&id=".$post_id."');</script>";
            }
            else if ($redirect == "columnists") {
                $baseurl = $base_url;
                echo "<script>window.location.replace('".$baseurl."columnists.php?action_request=edit_records&id=".$post_id."');</script>";
            }
            else {
