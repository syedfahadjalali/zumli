...
class General
{
    ...
    public
            $news                 = [self::LANG_ENGLISH => 'general_news', self::LANG_ARABIC => 'general_news_a'];
	public
            $sponsors             = [self::LANG_ENGLISH => 'sponsors', self::LANG_ARABIC => 'sponsors']; //1
	public
            $social_links             = [self::LANG_ENGLISH => 'social_links', self::LANG_ARABIC => 'social_links_a']; //1
			
	public
            $regid             = [self::LANG_ENGLISH => 'regid', self::LANG_ARABIC => 'regid']; //1
    public
    ...
    public
            function getImages($post_id, $post_type, $lan=0)
    {
		if($lan==0){
        $sql         = "SELECT concat('" . IMAGE . "', image) as image FROM app_images where post_id={$post_id} and post_type={$post_type}";
		}else{
			$sql         = "SELECT concat('" . IMAGE . "', image) as image FROM app_images_a where post_id={$post_id} and post_type={$post_type}";
		}
    ...
    public
            function getRaceCount($race_course_id,$lan)
    {
		if($lan==0){
        $sql        ="SELECT COUNT(id) as race_count from race where race_course_id={$race_course_id} AND is_active='1'";
		}else{
			 $sql        = "SELECT COUNT(id) as race_count from race_a where race_course_id={$race_course_id} AND is_active='1'";
		}
    ...
    {
        echo json_encode(array('flag' => $code, 'message' => $message, 'data' => $data), JSON_HEX_QUOT | JSON_HEX_TAG);
    }
